...
void MainWindow::ftpCommandFinished(int, bool error)
{
    if(ftp->currentCommand() == QFtp::ConnectToHost) {
        if (error){
            ...
            ui->connectButton->setText(QStringLiteral("已连接"));
            ui->label->setText(QStringLiteral("连接到服务器成功"));
        }
        ...
        if(error) ui->label->setText(QStringLiteral("下载出现错误：%1").arg(ftp->errorString()));
        else {
            file->close();
        ...
        if (isDirectory.isEmpty())
        {
            ...
            ui->label->setText(QStringLiteral("该目录为空"));
        }
    }else if (ftp->currentCommand() == QFtp::Put) {
        if(error) ui->label->setText(QStringLiteral("上传出现错误：检查文件是否重名！").arg(ftp->errorString()));
        else {
            ui->label->setText(QStringLiteral("上传完成"));
            file->close();
            currentIndex ++;
            if(currentIndex < indexCount){
        ...
        ui->label->setText(QStringLiteral("新建文件夹完成"));
        //刷新显示列表
        isDirectory.clear();
        ui->fileList->clear();
        ftp->list();
        ...
        currentIndex++;
        if(currentIndex >= indexCount){
            ui->label->setText(QStringLiteral("删除完成！"));
            isDirectory.clear();
            ui->fileList->clear();
            ftp->list();
        }
...
void MainWindow::on_downloadButton_clicked()
{
    ...
    if (!index.isValid()) {
        ui->label->setText(QStringLiteral("请选择正确的下载路径！"));
        return;
    }
    //获取下载文件信息
...
void MainWindow::slotDeleteFile()
{
    ...
    ui->label->setText(QStringLiteral("正在删除中......请稍等！"));
